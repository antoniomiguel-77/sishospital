...
<div>
              ...
              <div class="card shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <table class="table table-bordered table-hover text-center">
                        ...
                        <tbody>
                            @if (isset($atendentes) and $atendentes->count() > 0)
                            @foreach ($atendentes as $item)
                            <tr>
                                <td>{{$item->nomeCompleto}}</td>
                                <td>{{$item->idade ?? 'NÃO DEFINIDA'}}</td>
                                <td>{{$item->contribuente}}</td>
                                <td>{{$item->telefone}}</td>
                                <td>
                                    <button type="button" wire:click="editar({{$item->id}})" data-bs-toggle="modal" data-bs-target="#atendente" class="btn btn-sm btn-outline-primary"> <i class="fa fa-info-edit"></i></button>
    ...
    @include('livewire.administrador.modal.atendente')
</div>
<script>
    document.addEventListener('fecharModal', () => {
     $('#atendente').modal('hide');
    }) 
</script>
